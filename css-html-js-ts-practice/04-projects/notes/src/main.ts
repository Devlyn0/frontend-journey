...
import { initNotes, renderNote, renderNewNote, removeNote, renderNewTag, clearNotes, parseNotes, parseNewNotes, saveNotes } from "./notes";
...
notesContainer.addEventListener('click', (e) => { // Удаление заметки
...
notesContainer.addEventListener('click', (e) => { // Добавление нового тега
    if (!(e.target instanceof HTMLElement)) {
        return;
    }
...
notesContainer.addEventListener('click', (e) => { // Сохранение новой заметки
    if (!(e.target instanceof HTMLElement)) {
        return;
    }
    ...
    let notesToSave: Note[] | null = null;
...
notesContainer.addEventListener('mousedown', (e) => { // Drag`n`Drop
    if (!(e.target instanceof HTMLElement)) {
        return;
    }
