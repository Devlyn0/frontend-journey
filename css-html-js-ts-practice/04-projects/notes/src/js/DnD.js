...
function orderHandle(noteMiddlePointX, noteMiddlePointY, transparentClone, event) {
    ...
    if (!(elemUnderDragNote instanceof HTMLElement))
        return;
    target.style.display = '';
    const noteUnderDrag = elemUnderDragNote.closest('.note');
    if (!(noteUnderDrag instanceof HTMLElement))
        return;
    if (noteUnderDrag.id === 'transparent-drag' || noteUnderDrag.dataset.inAnimation) {
        return;
    }
    ...
    animateMove(noteUnderDrag, transparentClonePos.left, transparentClonePos.top, animateDuration);
    animateMove(transparentClone, noteUnderDragPos.left, noteUnderDragPos.top, animateDuration, () => {
...
function dragHandle(note, event, storage) {
    // Пайплайн начала DnD
    ...
    const mouseUpHandler = () => {
        ...
        animateMove(note, transparentPos.left, transparentPos.top, 0.5, animateCallback);
    };
